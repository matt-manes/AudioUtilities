...
TEST_CLASS(RampTest){
    public :
        ...
        TEST_METHOD(TestReverseRamp){
            AudioUtilities::Ramp::Ramp ramp = AudioUtilities::Ramp::Ramp(500.0f, 1000, 1.0f, 0.0f);
        ...
        TEST_METHOD(TestExpRamp){
            AudioUtilities::Ramp::Ramp ramp = AudioUtilities::Ramp::Ramp(500.0f, 1000, 0.0f, 1.0f, 0.1f);
            for (int i = 0; i < 1000; i++){
			    if (i < 100){
                    Assert::IsFalse(ramp.isActive());
                    Assert::AreEqual(ramp.getStart(), ramp.getCurrentVal());
